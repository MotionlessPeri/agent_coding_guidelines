...
## 三种写法 × 两种文件 —— 前两种各对一半，而对的那一半正好互补

实测 **Python 3.10.18 与 3.12.9**（两版结果逐格相同）/ Windows：

| 写法 | 文件本来是 CRLF | 文件本来是 LF |
|---|---|---|
| ① `read_text()` + `write_text()` | ✅ 原样 | 🛑 **被转成 CRLF** |
| ② `read_text()` + `write_text(newline="")` | 🛑 **被转成 LF** | ✅ 原样 |
| ③ `open(newline="")` 读 + 写 | ✅ 原样 | ✅ 原样 |
| 编辑器 / Write / Edit 类工具 | ✅ 不经这条翻译层 | ✅ 同 |

🎯 **①② 各只在一半情形下对，而它们对的那一半正好互补。** 所以两个人各拿一半样本试，
会各自得出一条「我这个写法是对的」——而**把两条合起来用，就产生一条错的**。
⇒ 这才是它值得写下来的理由：它不是「Windows 行尾」那条常识，是
**「两条各自正确的做法混用会产生一条错的」**的一个可复现实例。

⚠️ **`Path.read_text` 不收 `newline`**（3.10.18 / 3.12.9 实测均不收，而同版本的 `write_text`
**收**）⇒ 想让读写两侧都不翻译，**只能用 `open()`**，pathlib 没有对应写法。
更高版本有没有加，本机没有可跑的解释器 ⇒ **未测**。
...
- **单项目、一天内四次独立命中**（四种错法各一次）；上面那张矩阵在 **3.10.18 与 3.12.9** 两个解释器上各跑一遍，逐格相同。
