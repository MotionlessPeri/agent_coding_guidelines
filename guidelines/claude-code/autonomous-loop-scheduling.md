...
Claude Code 另有一个原生的「朝目标自主推进」命令 `/goal`,跟本条是**两个不同机制,按任务形态选**,不是二选一的优劣。对比如下(`/goal` 一栏据[官方文档](https://code.claude.com/docs/en/goal);表里各项**非实测**,但 `/goal` 的三条**使用侧**实测事实见下一节):
...
选了 `/goal` 之后,条件文本怎么写决定它好不好用。下面三条都是实测撞出来的。

**① 条件文本会被整段 pin 在界面顶部 —— 写长了会把 agent 的全部输出挡住。**

这是最容易低估的一条,因为它跟任务能不能完成无关,只跟**用户能不能看到成果**有关。实测:一个 ~80 行的 `/goal` 条件(分两段任务 + 十条纪律)把可视区域占满,用户连续几轮**完全看不到** agent 的回复,最后只能让 agent 事后口头重述整轮工作。任务全做完了,但过程对用户不可见。

→ **条件写紧凑**:每个任务项一行标题 + 关键判据,纪律压成三五行。**细节放 durable 文档**(handoff / 进度文件)让 agent 自己去读,而不是全塞进条件文本。这跟本条前面「loop prompt 指向 durable SoT」是同一个道理,只是这里的驱动力是界面而非 compaction。

**② 条件必须可判定、可终止 —— 每项以「交付**或**明确 park」收口。**

Stop hook 会一直拦着不让停,所以开放式条件(「持续探索 X」「尽可能完善 Y」)会让它永不满足。可终止的写法是给每项两个出口:做成了,或者**明确 park 并写明卡在哪**。实测有效——4 项任务里 3 项交付、1 项在两轮否证后 park,条件照样判为达成。

**③ 把已踩出的纪律写进条件本身,能提高遵守率,但不保证。**

自主跑时没有用户在每轮拦一下,前几轮踩出来的纪律(先过 identity 闸门 / 扫旋钮前先做活性检查 / 报了 max 还要看分布)如果不写进条件,下一轮会原样重犯。实测把它们写进去确实防住了几次。

但**不是万能的**:同一轮里出现过「agent 在 probe 注释里刚写下『先看数量级合不合理再读含义』,下一段就拿一个数量级明显不合理的中间量下了结论」。所以纪律入条件是**必要不充分** —— 它减少重犯,不消除。真要硬保证得上确定性检查(见 [`../../techniques/fact-forcing-gate.md`](../../techniques/fact-forcing-gate.md) 关于 advisory vs hard 的那节)。

> 这三条只在一个项目上实测过(单 session、多轮、有用户在旁观察),**跨项目未验证**。①最硬(纯 harness UI 行为),③最软(是倾向不是机制)。
