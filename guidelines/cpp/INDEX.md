...
| [`build-incremental-and-cmake.md`](build-incremental-and-cmake.md) | 改跨 DLL 公共头后增量编译漏重编 → ABI 布局不一致崩（容器/虚函数），`--clean-first` 全量重编;`git mv` / 改 CMakeLists 目录树后 VS IDE 用 stale `.vcxproj`（命令行能编、IDE 报 `cmake_pch.cxx not found`）→ 删**整个** build 目录重 cmake;依赖与消费方**工具集混链** → 缺 STL 向量化算法内部符号（`__std_find_last_trivial_1` 等 LNK2001），VS 默认把 `v143` 解析成旧版本 → 用 `Directory.Build.props` 的 `VCToolsVersion` 钉死（CMakePresets 的 `toolset` 吃不下版本号） |
