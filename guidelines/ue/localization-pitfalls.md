...
- skill `ue-settings-persistence` （`skills/ue/ue-settings-persistence/SKILL.md`）—— Trap 4 是 settings 持久化的特例；通用 UPROPERTY(config) 持久化 pattern 在那篇
- `skills/ue/ue-module-architecture/SKILL.md` —— Trap 1 修法（query API 暴露 culture-aware wrap）也是 Editor / Runtime API 边界设计（已 promote 到 skill `ue-module-architecture`）
