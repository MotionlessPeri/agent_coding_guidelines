...
| ProfileGPU / stat GPU | 1.1 | `Engine/Source/Runtime/RenderCore/Private/GPUProfiler.cpp` | P0 | 无 |
...
| FGlobalShader 编写 | 2.1 | `Engine/Source/Runtime/Engine/Public/GlobalShader.h` | P0 | HLSL、C++ |
...
| 自定义材质系统 | 2.4 | `Engine/Source/Runtime/Engine/Private/HLSLMaterialTranslator.cpp` | P1 | 材质系统、HLSL |
| Substrate 材质 | 2.4 | `Engine/Shaders/Private/Substrate/SubstrateBSDF.ush` | P1 | PBR 理论 |
| GBuffer 扩展 | 2.4 | `Engine/Source/Runtime/Renderer/Private/GBufferInfo.h` | P1 | GBuffer 布局 |
...
| Metal RHI | 3.3 | `Engine/Source/Runtime/MetalRHI/Private/MetalRHI.cpp` | P1 | Metal 基础 |
...
| 渲染管线裁剪 | 3.6 | `Engine/Source/Runtime/Renderer/Private/ShaderCompiler.cpp` | P0 | 渲染管线架构 |
...
| Shader 开发模式 | 4.3 | `Engine/Source/Runtime/Engine/Private/ShaderCompiler.cpp` | P0 | Shader 编译 |
...
| 知识卡片 | 关联领域 | 关键源码文件 | 学习优先级 | 前置知识 |
|----------|----------|-------------|-----------|---------|
| 三级渲染流水线 | 5.1 | `Engine/Source/Runtime/Renderer/Private/RenderingThread.cpp` | P0 | 多线程基础 |
...
| HLSL 生成 | 5.4 | `Engine/Source/Runtime/Engine/Private/HLSLMaterialTranslator.cpp` | P0 | HLSL |
| Shader 编译管线 | 5.4 | `Engine/Source/Runtime/Engine/Private/ShaderCompiler.cpp` | P0 | 编译原理 |
| Shader Permutation | 5.4 | `Engine/Source/Runtime/Engine/Public/ShaderPermutation.h` | P0 | 无 |
...
| Nanite 核心架构 | 5.6 | `Engine/Source/Runtime/Engine/Public/NaniteDefinitions.h` | P1 | 几何处理基础 |
...
|--------------------------------------|-----------|-----------|-------------|
| `Runtime/Renderer/Private/SceneRendering.cpp` | 渲染管线调度 | 渲染入口，FSceneRenderer::Draw 调度所有 Pass | `FSceneRenderer::Draw`, `PreRender`, `PostRender`, `InitViews` |
| `Runtime/Renderer/Private/DeferredShadingRenderer.cpp` | 延迟渲染 | 延迟着色主流程，RenderBasePass → RenderLights → PostProcessing | `FDeferredShadingSceneRenderer::Render` |
| `Runtime/Renderer/Private/MobileShadingRenderer.cpp` | 移动端渲染 | 移动端渲染主流程 | `FMobileSceneRenderer::Render` |
| `Runtime/Renderer/Private/SceneVisibility.cpp` | 可见性计算 | InitViews 核心实现，可见性裁剪/阴影分配/Lumen 场景更新 | `InitViews`, `FRelevancePacket`, `ComputeAndMarkRelevanceForViewParallel` |
| `Runtime/Renderer/Private/BasePassRendering.cpp` | GBuffer 写入 | BasePass 实现，渲染不透明 Mesh 到 GBuffer | `RenderBasePass`, `FBasePassMeshProcessor` |
| `Runtime/Renderer/Private/LightRendering.cpp` | 光照计算 | Lighting Pass 实现，逐像素光照计算 | `RenderLights`, `FDeferredLightSceneInfo` |
| `Runtime/Renderer/Private/ShadowRendering.cpp` | 阴影渲染 | Shadow Map 渲染 | `RenderShadowDepthMaps`, `FProjectedShadowInfo` |
| `Runtime/Renderer/Private/PostProcess/PostProcessing.cpp` | 后处理 | 后处理管线，Bloom/Tonemapping/TSR/DOF/MotionBlur | `FPostProcessing::Process`, `AddPostProcessingPasses` |
...
| `Runtime/RenderCore/Public/RenderGraphBuilder.h` | RDG 核心 | RDG 核心 API，Pass 注册/资源创建/执行 | `FRDGBuilder`, `AddPass`, `CreateTexture`, `Execute` |
| `Runtime/RenderCore/Public/RenderGraphResources.h` | RDG 资源 | RDG 资源抽象，Texture/Buffer 描述符 | `FRDGTexture`, `FRDGBuffer`, `FRDGResource` |
| `Runtime/RenderCore/Public/RenderGraphPass.h` | RDG Pass | Pass 类型体系 | `FRDGPipelineStatePass`, `ERDGPassFlags` |
| `Runtime/RenderCore/Private/RenderGraph.cpp` | RDG 执行 | RDG Compile/Execute 实现 | `FRDGBuilder::Execute`, Compile, Culling |
| `Runtime/RenderCore/Private/RenderGraphAllocator.cpp` | RDG 内存管理 | Transient 资源分配与别名优化 | `FRDGTransientResourceAllocator` |
| `Runtime/RenderCore/Private/RenderGraphValidation.cpp` | RDG 调试 | RDG 调试验证 | RDG Validation |
| `Runtime/RHI/Public/RHICommandList.h` | RHI 命令 | RHI 命令列表核心定义 | `FRHICommandList`, `FRHICommandListImmediate` |
| `Runtime/RHI/Public/DynamicRHI.h` | RHI 抽象接口 | 平台无关 RHI 抽象基类 | `FDynamicRHI` (数百个纯虚函数) |
| `Runtime/RHI/Public/RHIAccess.h` | RHI 屏障 | Barrier/Transition API | `ERHIAccess`, `FRHITransitionInfo`, `FRHIBarrier` |
...
| `Runtime/Renderer/Private/RenderingThread.cpp` | 渲染线程 | 渲染线程启动与控制 | `StartRenderingThread`, `FRenderCommandFence` |
| `Runtime/D3D12RHI/Private/D3D12RHI.cpp` | D3D12 RHI | D3D12 平台 RHI 实现 | `FD3D12DynamicRHI::Init` |
| `Runtime/D3D12RHI/Private/D3D12CommandContext.cpp` | D3D12 命令 | D3D12 命令上下文与 Barrier | `FD3D12CommandContext::RHITransitionResources` |
| `Runtime/D3D12RHI/Private/D3D12Resource.cpp` | D3D12 资源 | D3D12 资源管理 | `FD3D12Resource`, DeferredDelete |
| `Runtime/VulkanRHI/Private/VulkanRHI.cpp` | Vulkan RHI | Vulkan 平台 RHI 实现 | `FVulkanDynamicRHI::Init` |
| `Runtime/VulkanRHI/Private/VulkanCommandList.cpp` | Vulkan 命令 | Vulkan 命令列表 | `FVulkanCommandListContext` |
| `Runtime/VulkanRHI/Private/VulkanPipelineState.cpp` | Vulkan PSO | Vulkan PSO 缓存 | `FVulkanPipelineStateCache` |
| `Runtime/Engine/Public/MaterialShared.h` | 材质系统 | FMaterial/FMaterialResource/FMaterialShaderMap | `FMaterial`, `FMaterialResource`, `FMaterialShaderMap` |
| `Runtime/Engine/Public/Material.h` | 材质资产 | UMaterial UObject 定义 | `UMaterial`, `UMaterialInstance` |
| `Runtime/Engine/Private/HLSLMaterialTranslator.cpp` | HLSL 生成 | 材质表达式 → HLSL 代码生成 | `FMaterialCompiler` |
| `Runtime/Engine/Public/ShaderCompiler.h` | Shader 编译 | Shader 编译任务与队列 | `FShaderCompileJob`, `FShaderCompilingThreadManager` |
| `Runtime/Engine/Private/ShaderCompiler.cpp` | Shader 编译调度 | 主进程编译调度 | `FShaderCompilingThreadManager` |
| `Runtime/Programs/ShaderCompileWorker/ShaderCompileWorker.cpp` | SCW 进程 | Shader 编译外部进程入口 | `main` |
| `Runtime/Engine/Public/ShaderPermutation.h` | Permutation 系统 | Shader Permutation 定义与裁剪 | `FShaderPermutationBool`, `TShaderPermutationDomain` |
| `Runtime/Engine/Public/GlobalShader.h` | Global Shader | FGlobalShader 基类与注册宏 | `FGlobalShader`, `IMPLEMENT_GLOBAL_SHADER` |
| `Runtime/Engine/Public/ShaderCore.h` | Shader 核心 | Shader 参数反射系统 | `SHADER_PARAMETER_STRUCT`, `FShader` 基类 |
| `Runtime/Engine/Public/ShaderParameters.h` | Shader 参数 | SHADER_PARAMETER_STRUCT 宏定义 | `BEGIN_SHADER_PARAMETER_STRUCT` |
| `Runtime/Engine/Public/RHI.h` | RHI 枚举 | ERHIFeatureLevel/ERHIShaderPlatform 等枚举 | `ERHIFeatureLevel`, `IsFeatureLevelSupported()` |
| `Runtime/Engine/Public/SceneView.h` | 场景视图 | FSceneView/FViewInfo 定义 | `FSceneView`, `FViewInfo`, `EStereoscopicPass` |
| `Runtime/Engine/Private/SceneView.cpp` | 视图计算 | 视图矩阵/投影矩阵计算 | `FSceneView::SetupViewRect` |
| `Runtime/Engine/Public/NaniteDefinitions.h` | Nanite 定义 | Nanite 数据结构定义 | `Cluster`, `Page`, `Group` 结构体 |
...
| `Runtime/Renderer/Private/Lumen/LumenSceneRendering.cpp` | Lumen 场景 | Lumen 场景数据管理、体素化、主渲染调度 | `RenderLumenScene()`, `VoxelizeLumenScene()`, `ShouldRenderLumen()` |
| `Runtime/Renderer/Private/Lumen/LumenReflections.cpp` | Lumen 反射 | Lumen 反射合成与降噪 | `CompositeLumenReflections()`, `RenderLumenReflections()` |
...
| `Runtime/Renderer/Private/Lumen/LumenMeshCards.cpp` | Lumen Mesh Card | Mesh Card 生成与管理 | `LumenSceneCardBuild()`, `CardRasterize()` |
...
| `Runtime/RenderCore/Private/GPUProfiler.cpp` | GPU 性能分析 | GPU Profiler 实现 | `FGPUProfiler` |
| `Runtime/RenderCore/Private/GPUCrashDebugging.cpp` | GPU Crash 诊断 | GPU Crash 调试 | `FGPUCrashDebugging` |
| `Runtime/RenderCore/Private/DumpGPU.cpp` | GPU 帧捕获 | DumpGPU 实现 | `DumpGPU` |
...
| `Runtime/RenderCore/Private/RenderGraphValidation.cpp` | RDG 验证 | RDG 验证实现 | RDG Validation |
| `Runtime/Engine/Private/Shader.cpp` | Shader 核心 | Shader 序列化/缓存/Permutation 调度 | `FShader`, `GET_SHADER_CONDITIONAL` |
| `Runtime/Engine/Private/MaterialShaderMap.cpp` | ShaderMap 编译 | ShaderMap 编译调度 | `FMaterialShaderMap::Compile` |
| `Shaders/Private/MaterialTemplate.ush` | 材质模板 | 材质 HLSL 模板 | `GetMaterialPixelParameters()`, `GetMaterialVertexParameters()` |
...
**要读的源码**：
- `Runtime/Renderer/Private/SceneRendering.cpp` — `FSceneRenderer::Draw` 入口函数
- `Runtime/Renderer/Private/DeferredShadingRenderer.cpp` — `Render` 主流程
- `Runtime/Renderer/Private/RenderingThread.cpp` — 三级流水线启动
- `Runtime/RHI/Public/RHICommandList.h` — `FRHICommandList` 定义
...
**要读的源码**：
- `Runtime/RenderCore/Public/RenderGraphBuilder.h` — `FRDGBuilder` 核心 API
- `Runtime/RenderCore/Public/RenderGraphResources.h` — `FRDGTexture` / `FRDGBuffer`
- `Runtime/RenderCore/Public/RenderGraphPass.h` — `ERDGPassFlags` / Pass 类型
- `Runtime/RHI/Public/RHIAccess.h` — `ERHIAccess` / `FRHIBarrier` / `FRHITransitionInfo`
- `Runtime/Renderer/Private/PostProcess/PostProcessing.cpp` — 后处理链的 RDG 编排参考
...
**要读的源码**：
- `Runtime/Renderer/Private/Lumen/LumenSceneRendering.cpp` — `ShouldRenderLumen()`、`RenderLumenScene()`
- `Runtime/Renderer/Private/Lumen/LumenReflections.cpp` — `CompositeLumenReflections()`
...
- `Runtime/RenderCore/Private/GPUProfiler.cpp` — GPU Profiler 内部机制
...
**要读的源码**：
- `Runtime/RHI/Public/RHI.h` — `ERHIFeatureLevel` / `IsFeatureLevelSupported()`
- `Runtime/Renderer/Private/MobileShadingRenderer.cpp` — Mobile 渲染路径
- `Runtime/Renderer/Private/ShaderCompiler.cpp` — `ShouldCompilePermutation()`
- `Runtime/Engine/Public/ShaderPermutation.h` — `FShaderPermutationBool` 裁剪机制
...
**要读的源码**：
- `Runtime/Engine/Private/HLSLMaterialTranslator.cpp` — 材质表达式 → HLSL 完整流程
- `Runtime/Engine/Public/MaterialShared.h` — `FMaterialShaderMap` 序列化
...
- `Runtime/Renderer/Private/MeshPassProcessor.cpp` — `FMeshPassProcessor` 注册机制
