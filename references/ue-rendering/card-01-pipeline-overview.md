...
> **注意**：这份文档在 2026-07-30 之前从未经过源码验证（原始调研产物，且曾因子 agent 输出
> 错位而在仓库里丢失过一段时间，从 `bc4338a` 取回）。路径与 CVar 已清，但**各 Pass 的先后
> 顺序、线程同步描述、函数级流程叙述尚未逐条对照源码**——这部分是本文档的主体内容，
> 引用前请按 [`card-13-verify-locally.md`](card-13-verify-locally.md) §5 从 Pass 名反查注册点确认。
