...
description: 用于产出非平凡代码改动后的对话收尾，或用户要求 walkthrough、结构化 review、重构检查、正确性复核和注释整理时。默认检查本对话改动的文件与函数结构、超长函数、重复代码、风险边界、注释稳定性和接口契约，并在用户确认后执行重构与验证。平凡机械改动或用户明确把 review 推迟到后续里程碑时跳过。
---
