...
| Looping on one failed method | Stop and escalate when the same root cause and failure phenomenon survive three consecutive targeted repairs. |
| Aggregating unrelated findings | Start a separate failure-signature ledger; review and remediation have no cumulative cap across distinct findings. |
| Treating `repeat until approved` as permission to repeat one failed method forever | Continue only through a genuinely different authorized approach or user direction after the same signature survives three targeted repairs. |
