...
description: Use when designing, extracting, or reviewing a system where multiple plugins or modules share one core entity; when deciding how optional features attach data without subclass explosion; when separating presets, runtime instances, snapshots, operations, and lookup registries; or when an authoritative shared type cannot be extended. Skip for single-plugin systems with no shared-core extensibility need.
---
