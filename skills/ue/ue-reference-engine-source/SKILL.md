...
description: Meta prep-work rule for UE — before implementing any UE feature (especially UI / Editor / Asset Editor / Slate / Localization / SourceControl / Validator / Subsystem / Factory / Details Customization), first locate the closest reference implementation in UE engine source / official samples and read it. UE framework is largely undocumented; correct usage patterns live in engine source, not in API docs. Bundles a 5-tier priority list (engine native → samples → third-party plugins → docs/forums → AI last), a categorized reference table indexed by ~22 UE subsystems pointing at the right engine source files, 4 anti-patterns (assumption-driven coding, unverified AI code, forum copy-paste, pushing past missing reference), and 11 project-validated examples. Use BEFORE writing new UE code; symmetric counterpart to `reuse-before-implementing` (which surveys project-internal code).
when_to_use: Fires before writing any non-trivial UE C++ code — especially when (1) authoring a new UEdGraph / Schema / SGraphNode / Asset Editor / Slate widget / UAssetDefinition / Factory / Editor Validator / Subsystem / Details Customization, (2) overriding any UE framework virtual method for the first time, (3) implementing a UE 5.x feature you have not done before, (4) diagnosing a UE crash / weird behavior to find how the engine intends it to work, (5) reviewing AI-generated UE code (each API must be grep-verified against engine source). Skip for trivial mechanical edits where the existing pattern in the same file is already the reference.
---
...
- `skills/ue-module-architecture/SKILL.md` — module / layer decisions often happen at the same prep stage as reference lookup.
