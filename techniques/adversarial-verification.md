...
## 判据的三个隐藏前提：判据来自哪儿 / 测在哪个构型 / 敏感区间在哪

上面那节管"判据可不可信"。这节管**一条已经绿着的判据,它到底证明了什么** —— 三个前提各自失效时,
**表现都是"绿"**,所以只能事先知道。

### 一、判据的**来源**决定它错的时候会不会红

| 判据来自 | 理解错了会怎样 |
|---|---|
| **外部 oracle**(上游源码 / round-trip / 参照实现 / 不变量) | 🟢 **当场红** —— 判官不是我的理解 |
| **我自己的理解** | 🔴 **跟着一起错,而且绿** |

> **一条绿着的断言,证明的是"代码跟作者当时的理解一致",不是"这个理解对"。**
> ⚠️ **而它比沉默更糟**:下一个人读到它会**更确信**那个错误理解 —— 有一条通过的测试撑腰,
> **排查时他会最后才怀疑那条测试。**

⇒ **写一条"锁行为"的断言之前,先问它的判据来自哪儿。** 来自上游 ⇒ 锁;来自自己的理解 ⇒
**那不是锁,是把当前理解固化,而固化一个错误理解比没有断言更贵。**

⚠️ **但不要给每条断言都标来源** —— 判据是**标"信任级别不显然"的那条**:一屏 🟢 里的一个 🟡 才看得见,
全标反而把信号淹掉,还造出一批会漂移的副本。

### 二、测量的**构型**必须跟被断言的场景一致 —— 而"有实测"不等于"实测对"

在用户不会遇到的构型里量出来的结论,**跟没量过一样不可靠,而且更有说服力**(它带着数字)。

⚠️ 最贵的形态不是"没量",是 **"量了,但 regime 错"** —— 因为**它更难被质疑**。实测的一句面板文案演化:
v1"从字面推的"(没量) → v2"有实测但量在错构型上"(**结论比 v1 更错,而它带着数字**) → v3(位移那半退回 v1 的说法 + 补上另两半,三条各带实测与对照)。

⇒ **一条可查表的判据**:先分清哪些量对构型**等变**。引用绝对世界方向的(地面高度 / 某个轴当前向 /
按水平-垂直分轴)**不等变,数字必须连构型一起记**;其余等变的不必。
⚠️ 这同时解释了坏构型为什么能潜伏很久:**大部分测量看不出差别。**

### 三、判据的**敏感区间**要从被测对象自己的量算出来,别写死

一个判别窗口通常是**区间**,而**区间两侧的失败长得一模一样**(都是"两条路一致 ⇒ 没有缺陷")。

实测:钉一个"预览与烘焙不一致"的缺陷,阈值扫下来 —— `10/50/100` 两边都从不触发 ⇒ 一致;
`200/500/2000` **缺陷显形**;`20000/1e6` 两边都触发 ⇒ 又一致。作者第一版取窗口**上方**、第二版取**下方**,
**两次都绿、两次都差点报"缺陷不成立"**。

⇒ **阈值从实测量算**(如"真实最大速度 × 1.5"),夹具变了它跟着走。**写死一个数就是把这个坑埋给下一个人。**
⇒ 跟"门槛要相对于机制自己的动态范围"是一对:那条管**下界**(别把惰当生效),这条管**整个区间**。

**诚实边界**:三条出自同一批跨对话协作(两项目、三方),机制清楚可复述,**未跨人 / 跨团队验证**。apply-and-refine。
...
- 附一条**报数精度**的：**一个数该给几位，取决于两次独立计算能重合到第几位，不取决于计算器能吐几位。** 实测：同一对比率两方独立算得 6.26 与 6.23（合成取整时机不同）⇒ 第二位小数是假精度，表里统一降到一位；而同表的 1.05 / 1.08 保留两位——它们要显示的是「离 1.00 有多近」。⇒ 保留几位跟「这个数要回答什么」绑定，不是全表一刀切。（单项目一击。）
...
### ⭐ 先证明那道**门**能红，再信它的绿 —— fail-open 的判据比坏掉的量具更贵

上面整节讲「量具先自证」，对象是**我用来测的那个东西**。还有一层要单独说，因为它的**后果不对称**：

| | 对象 | 坏掉时产出什么 | 会被质疑吗 |
|---|---|---|---|
| 量具先自证 | 我用来测的东西 | 一个**错读数** | ✅ 会 —— 数字可疑就有人查 |
| **先证门能红** | **判决用的那道门**（审计门 / lint / CI gate / 校验脚本） | 一个**绿** | 🛑 **不会** —— 绿正是大家想要的结果 |

⇒ 🎯 **所以 fail-open 的判据比坏掉的量具更贵：它的失效形态恰好是「没有问题」。**
⇒ ✅ 唯一兜得住的动作：**造一个已知违规跑一遍，看它红**，再信它的绿。

#### 🛑 而它失效的方式可以是「每一层看都合规」

实测（一个多轴结构审计门）：正确调用要四组参数（三个扫描根 + 一份 config + 一份 controls）。
少给时发生的事：

```
漏 --config / --controls  ⇒ **不报错**（它们是可选参数）
漏两个扫描根               ⇒ **不报错**（没根就没东西可扫）
⇒ 🛑 **两个各自「合法」的省略，合成一个结构上不可能红的门** —— 实测 exit 0、未判模块 95 个
   而正确调用是 exit 2（一条 invariant 红）
```

⇒ ⚠️ 这跟「某个参数写错了」完全不同：后者会报错，前者**在每一层看都合规**。
⇒ 📌 而诱因可复现：**README 里那条命令带续行符（`\`），只读第一行就用** ——
　而命令的**后半截**恰好是"让门真的去看东西"的那部分。⇒ **从文档抄命令时先确认抄全了**；
　带续行符的命令，"只读第一行"这个错的表现是**一个语法完全合法、退出码为 0 的调用**。

#### ⚠️ 一条配套的诚实边界

「这道门今天是绿的」和「这道门能红」是两个断言。**被动撞到它红过一次**（别人先报、你复现）
也**不等于**你证过它能红 —— 那次红是那次的被测对象给的，不是你造的。
⇒ 所以引用一道门的绿之前，问一句：**我（或任何人）主动让它红过吗？**

**诚实边界**：单项目、一次（一个多轴结构审计门 + 它的 controls 组）。机制清楚可复述、且失效形态
（fail-open + 合法省略叠加）不依赖具体工具 ⇒ apply-and-refine。
...
### 同型故障:探针的失效模式跟被测性质同型 ⇒ 指纹表整族失效

上面那张「数字以结构化方式可疑」的指纹表有一个共同前提:量具坏掉时,读数**长得跟正确读数不一样**。有一类故障不满足这个前提——**探针自己的失效模式,跟要测的那个性质是同一个**:

```
测「会不会静默失败」,而探针带 2>/dev/null(它自己制造静默)   ⇒ 必然测出「静默」
测「行尾变了吗」,而用会归一化行尾的比较口径                 ⇒ 必然测出「没变」
测「反引号配对断没断」,而围栏判定自己错读反引号语境           ⇒ 必然测出一批假命中
```

跟一般量具故障的决定性差别:一般故障的读数**可疑**(恰好为 0 / 太整齐 / 两边矛盾),**同型故障的读数恰好确认你要测的那件事 ⇒ 零可疑感**——整张指纹表对它结构性失效。可用的只有两个动作:

1. **事前一问**:「我这个探针的失效模式,跟我要测的那个性质,是不是同一个?」
2. **检测**:喂一个**已知具有性质 P** 的样本(⚠️ 不是「已知正常」的样本——那查不出同型故障,实测)。门槛比听起来低:P 通常是失败模式,而**失败可以注入**。

⇒ 推论:**「造红」要对两类对象做——门(被检查的东西)和量具(做检查的东西)。**只给门造红时,一个坏量具产出的读数跟正常读数逐字相同,而门的绿是假的。(上面「先证明那道门能红」管门;本节管量具那半。)

**诚实边界**:单项目、单日三击,三个不同技术域(shell 静默 / 行尾口径 / 文本围栏判定),同一协作批次——表层各异、机制同一,但未跨项目验证 ⇒ apply-and-refine。
