...
- ✅ 已促成(2026-08-07) 动态 workflow 命名模式 Tournament / Generate-and-filter / Classify-and-act `[已核源+修正:blog 实际命名 6 种非 worker 报的 7 种;Quarantine 不是并列命名模式,是 Triaging at Scale 节里一句做法]` —— coordination-patterns.md Dependency Patterns 加「结果选优与路由」小节（三模式 + filter/tournament 分界 + Quarantine 以做法注记）。源:claude.com/blog/a-harness-for-every-task-dynamic-workflows-in-claude-code
- 👁 观察(2026-08-07 评审) Agent Teams（v2.1.178+ 实验）`[存在性已交叉确认,细节未核]` —— 定性:**team=有界任务组,lane=常驻角色**,不是 role-lane 的替代而是机制底座候选。最先该被它替的是 `multi-session-coordination` 的手搓机制层（lease hook/inbox 注入 vs 官方共享任务列表+文件锁自领+SendMessage）;方法层（role⊥task/oracle 分档/brief 纪律）与机制无关不受影响。**试用触发**:下次出现单 repo、有界、可并行的大活（迁移/批量审计）开一次实测,回答三问——①跨 repo 行不行 ②teammate 生命周期是否绑 lead 会话（决定够不够到常驻 lane）③plan gate 之外能否塞分档 oracle。零使用前不进 corpus。源:code.claude.com/docs/en/agent-teams
- 👁 观察(2026-08-07 已核源:三阶段逐字属实) Auto mode classifier 对 subagent 三阶段检查 —— ①spawn 前审 delegation prompt(v2.1.178+,拦的是"话"不是动作) ②运行期逐动作过审且 frontmatter `permissionMode` **静默无视** ③完成后复审动作史、可疑则**结果前置安全警告**(会打破 worker-instructions 的"首行回显+断言",需容忍前缀)。**不促成理由:仅 auto mode 生效,用户日常 bypassPermissions,零 friction**。触发:真用上 auto mode(cloud 会话/无人值守长任务)时促成进 subagent-contracts.md。源:code.claude.com/docs/en/permission-modes
- 👁 观察+评估任务(2026-08-07 已核源,两处修正:settings.json 仅支持 `agent`+`subagentStatusLine` 两键非任意配置;"v2.1.128" 页面无此版本号,worker 似真编造实锤一例) Plugin 系统打包分发 —— 核实:命名空间 `/plugin:skill`(与散装同名共存不覆盖)/`bin/` 入 Bash tool PATH/打包面 skills+agents+hooks+MCP+LSP+monitors/`--plugin-dir` 本地加载+`/reload-plugins` 免重启。**用户评估角度:按 scope 打包+启停**——sync-skills 扁平全量装,每项目会话常驻全部 20+ skill description(context-budget 未审计过的隐性项);plugin 按域打包(ue/maya/collab)+按需启停可砍掉跨域 description 常驻。**三门槛**:①Codex 双端(plugin 是 CC 独有,替代不了 sync-skills 只能并行双轨) ②命名空间改名成本 ③**per-project 启停粒度未核实**(价值前提,spike 第一问)。spike 已挂 open-items:`--plugin-dir` 拿 skills/maya/(2 skill 最小)试包。源:code.claude.com/docs/en/plugins
...
- ✅ 已促成(2026-08-07) settings.json 强制执行语义（enforced vs guidance）`[已核源:引语逐字属实]` —— 最轻形态：hook-conventions 开篇加「配置两层分界」注记（零例外规则放 enforcement 层 / 偏好别放），不单开文件；与 fact-forcing-gate advisory-hard 光谱互链。源:code.claude.com/docs/en/claude-directory
